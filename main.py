...
def highlightFace(net, frame, conf_threshold=0.7, is_video=True):
    # делаем копию текущего кадра/изображения
    frameOpencvDnn = frame.copy()
    # высота и ширина кадра/изображения
    ...
    for i in range(detections.shape[2]):
        ...
        if confidence > conf_threshold:
            ...
            # рисуем рамку на кадре/изображении
            cv2.rectangle(frameOpencvDnn, (x1, y1), (x2, y2), (0, 255, 0), int(round(frameHeight/150)), 8)
    # вовзращаем кадр/изображение с рамками
...
def processImage(image_path):
    ...
    # определяем лица в изображении
    resultImg, faceBoxes = highlightFace(faceNet, frame, is_video=False)
    # перебираем все найденные лица в изображении
    for faceBox in faceBoxes:
        ...
        cv2.putText(resultImg, f'{gender}, {age}', (faceBox[0], faceBox[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 4, (0, 255, 255), 8, cv2.LINE_AA)
